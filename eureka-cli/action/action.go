...
func New(name, gatewayURL string, runParams *actionparams.ActionParams) *Action {
	return newGeneric(name, gatewayURL, runParams, "", "", "")
}

func NewWithCredentials(name, gatewayURL string, runParams *actionparams.ActionParams, vaultRootToken, keycloakAccessToken, keycloakMasterAccessToken string) *Action {
	return newGeneric(name, gatewayURL, runParams, vaultRootToken, keycloakAccessToken, keycloakMasterAccessToken)
}

func newGeneric(name, gatewayURL string, runParams *actionparams.ActionParams, vaultRootToken, keycloakAccessToken, keycloakMasterAccessToken string) *Action {
	return &Action{
		Name:                              name,
		...
		ConfigApplicationName:             viper.GetString(field.ApplicationName),
		ConfigApplicationVersion:          viper.GetString(field.ApplicationVersion),
...
func GetGatewayURL(actionName string) (string, error) {
	...
	if gatewayURL == "" {
		return "", fmt.Errorf("cannot construct gateway url for %s platform", runtime.GOOS)
	}
...
func (a *Action) GetPreReservedPort() (int, error) {
	...
	if freePort == 0 {
		return 0, fmt.Errorf("cannot find free TCP ports in range %d-%d", a.ConfigApplicationPortStart, a.ConfigApplicationPortEnd)
	}
