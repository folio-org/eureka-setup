...
func (ts *TenantSvc) SetConfigTenantParams(tenantName string) error {
	...
	helpers.SetBool(configTenant, field.TenantsSingleTenantEntry, &ts.Action.ActionParams.SingleTenant)
	helpers.SetBool(configTenant, field.TenantsEnableEcsRequestEntry, &ts.Action.ActionParams.EnableECSRequests)
	helpers.SetString(configTenant, field.TenantsPlatformCompleteURLEntry, &ts.Action.ActionParams.PlatformCompleteURL)
